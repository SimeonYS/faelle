...
class FaellePipeline:

    # Database setup
    conn = sqlite3.connect('faelle.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `faelle`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM faelle WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO faelle (date, title, link, content)"
